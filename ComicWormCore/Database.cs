        ...
        private void CreateNewDataBase()
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection("Data Source=" + DatabaseFile))
            {
                using (SQLiteCommand command = new SQLiteCommand())
                {
                    ...
                    tb3.Columns.Add(new SQLiteColumn("MD5", ColType.Text, true, false, true, null));
            ...
            lock (LockDatabase)
            {
                using (SQLiteConnection connection = new SQLiteConnection("Data Source=" + DatabaseFile))
                {
                    using (SQLiteCommand command = new SQLiteCommand())
                    {
                        ...
                        string cmd = "delete from Comic where MD5='" + comic.MD5 + "';";
                        helper.Execute(cmd);
            ...
            lock (LockDatabase)
            {
                using (SQLiteConnection connection = new SQLiteConnection("Data Source=" + DatabaseFile))
                {
                    using (SQLiteCommand command = new SQLiteCommand())
                    {
                        ...
                        string cmd = "delete from Chapter where Comic='" + comic.MD5 + "';";
                        helper.Execute(cmd);

                        string cmd2 = "delete from Page where Comic='" + comic.MD5 + "';";
                        helper.Execute(cmd2);
            ...
            lock (LockDatabase)
            {
                using (SQLiteConnection connection = new SQLiteConnection("Data Source=" + DatabaseFile))
                {
                    using (SQLiteCommand command = new SQLiteCommand())
                    {
                        ...
                        string cmd = "delete from Chapter where MD5='" + chapter.MD5 + "';";
                        helper.Execute(cmd);
            ...
            lock (LockDatabase)
            {
                using (SQLiteConnection connection = new SQLiteConnection("Data Source=" + DatabaseFile))
                {
                    using (SQLiteCommand command = new SQLiteCommand())
                    {
                        ...
                        string cmd = "delete from Page where Chapter='" + chapter.MD5 + "';";
                        helper.Execute(cmd);
            ...
            lock (LockDatabase)
            {
                using (SQLiteConnection connection = new SQLiteConnection("Data Source=" + DatabaseFile))
                {
                    using (SQLiteCommand command = new SQLiteCommand())
                    {
                        ...
                        string cmd = "select * from Chapter where MD5='" + chapter.MD5 + "';";
            ...
            lock (LockDatabase)
            {
                using (SQLiteConnection connection = new SQLiteConnection("Data Source=" + DatabaseFile))
                {
                    using (SQLiteCommand command = new SQLiteCommand())
                    {
                        ...
                        string cmd = "select * from Page where MD5='" + page.MD5 + "';";
                        DataTable dt = helper.Select(cmd);
                        connection.Close();
                        if (dt.Rows.Count > 0)
                        {
